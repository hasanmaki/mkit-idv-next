...
class AccountResponse(BaseModel):
    ...
    order_name: str | None = Field(
        None,
        examples=["Operator 1"],
        description="Name of the order this account belongs to",
    )
    ...
    is_active: bool = Field(
        ..., examples=[True], description="Whether account is active"
    )
    is_processed: bool = Field(
        ..., examples=[False], description="Whether account has been processed"
    )
    balance_last: int | None = Field(
        None, examples=[50000], description="Last known balance"
    )
    ...
    last_balance_response: dict[str, Any] | None = Field(
        None, description="Raw balance check response"
    )
